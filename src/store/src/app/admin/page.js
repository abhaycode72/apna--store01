...
function OrdersTable({ orders: orderList, search = '', onUpdate }) {
  const filtered = orderList.filter((order) => `${order.id} ${order.customer} ${order.mobile} ${order.location}`.toLowerCase().includes(search.toLowerCase()));
  return (
    <div className="overflow-x-auto">
      <table className="w-full min-w-[900px] text-left text-xs text-slate-300">
        <thead className="bg-slate-900 text-slate-400 uppercase font-semibold"><tr><th className="p-3">Order ID</th><th className="p-3">Customer</th><th className="p-3">Contact</th><th className="p-3">Location</th><th className="p-3">Total</th><th className="p-3">Payment</th><th className="p-3">Status</th></tr></thead>
        <tbody className="divide-y divide-slate-800/60">
          {filtered.map((order) => <tr key={order.id} className="hover:bg-slate-900/50"><td className="p-3 font-mono font-bold text-purple-400">{order.id}</td><td className="p-3"><span className="font-semibold text-white">{order.customer}</span><span className="mt-1 block text-[10px] text-slate-500">{order.email}</span></td><td className="p-3">{order.mobile}</td><td className="p-3">{order.location}<span className="mt-1 block max-w-[180px] truncate text-[10px] text-slate-500">{order.address}</span></td><td className="p-3 font-semibold">₹{order.total}</td><td className="p-3"><span className={`rounded px-2 py-0.5 font-bold ${order.mode === 'COD' ? 'bg-amber-500/20 text-amber-400' : 'bg-blue-500/20 text-blue-400'}`}>{order.mode}</span><select value={order.paymentStatus} onChange={(event) => onUpdate(order.id, { paymentStatus: event.target.value })} className="mt-2 block rounded border border-slate-700 bg-slate-900 px-1 py-1 text-[10px] text-slate-300"><option>PAID</option><option>PENDING_COLLECTION</option><option>REFUNDED</option></select></td><td className="p-3"><select value={order.status} onChange={(event) => onUpdate(order.id, { status: event.target.value })} className="rounded border border-slate-700 bg-slate-900 px-2 py-1 text-xs font-semibold text-emerald-400"><option>ORDER_PLACED</option><option>PACKING</option><option>OUT_FOR_DELIVERY</option><option>DELIVERED</option><option>CANCELLED</option></select></td></tr>)}
        </tbody>
      </table>
...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    ...
    fetch('/api/admin/orders').then((response) => response.ok ? response.json() : null).then((result) => {
      ...
      setLiveOrders(result.orders.map((order) => ({
        ...
        total: order.total,
        mode: order.paymentMethod === 'cod' ? 'COD' : 'ONLINE',
  ...
  async function updateLiveOrder(orderId, updates) {
    const currentOrder = liveOrders.find((order) => order.id === orderId);
    const response = await fetch('/api/admin/orders', {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ orderId, ...updates, customer: currentOrder ? { name: currentOrder.customer, email: currentOrder.email, mobile: currentOrder.mobile, address: currentOrder.address, location: currentOrder.location } : undefined }),
    });
    ...
    <div className="flex min-h-screen bg-slate-900 text-slate-100">
      ...
      <main className="flex-1 space-y-6 overflow-y-auto p-6 lg:p-8">
        ...
        {activeSection === 'dashboard' && <>
          <button onClick={() => setActiveSection('support')} className="mb-6 flex w-full items-center justify-between rounded-2xl bg-gradient-to-r from-blue-600 to-indigo-600 p-5 text-white shadow-lg transition hover:scale-[1.01]">
            <div className="flex items-center gap-4">
              <div className="rounded-xl bg-white/20 p-3"><Headphones size={28} /></div>
              <div className="text-left">
                <h3 className="text-lg font-black">Customer Service Details</h3>
                <p className="text-sm font-medium opacity-90">{tickets.filter(t => t.status !== 'Resolved').length} pending queries requiring attention</p>
              </div>
            </div>
            <span className="hidden sm:inline font-bold">View Full Details →</span>
          </button>
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">{stats.map(({ label, value, icon: Icon, change }) => <div key={label} className="rounded-2xl border border-slate-800 bg-slate-950 p-5"><div className="mb-2 flex items-center justify-between text-slate-400"><span className="text-xs font-semibold">{label}</span><Icon size={20} className="text-purple-400" /></div><div className="text-2xl font-black text-white">{value}</div><span className="mt-1 inline-block text-[11px] font-bold text-emerald-400">{change}</span></div>)}</div><div className="grid grid-cols-1 gap-6 lg:grid-cols-3"><section className="space-y-4 rounded-2xl border border-slate-800 bg-slate-950 p-6 lg:col-span-2"><div className="flex items-center justify-between"><h2 className="text-lg font-bold">Live Orders Stream</h2><button type="button" onClick={() => setActiveSection('orders')} className="text-xs font-bold text-purple-400 hover:text-white">View all</button></div><OrdersTable orders={liveOrders} onUpdate={updateLiveOrder} /></section><Alerts /></div></>}
        {activeSection === 'orders' && <Panel title="Orders" subtitle="Review and edit customer, payment and delivery details for every live order."><div className="mb-4 flex items-center gap-2 rounded-xl border border-slate-800 bg-slate-900 px-3 py-2 sm:max-w-sm"><Search size={17} className="text-slate-500" /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search order, customer or mobile" className="w-full bg-transparent text-sm outline-none placeholder:text-slate-500" /></div><OrdersTable orders={liveOrders} search={search} onUpdate={updateLiveOrder} /></Panel>}
